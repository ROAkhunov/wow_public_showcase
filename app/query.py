...
#:
#: Коэффициента охвата и свежести здесь нет с 03.09 (T-86), и дело не в
#: пропусках: заполнены обе колонки почти всюду. Коэффициент — отношение охвата
#: к подписчикам, и на канале с одним подписчиком он вырождается: голову списка
#: занимали каналы на 1-6 подписчиков с коэффициентом в миллионы процентов.
#: `last_post_at` у карточки, не прошедшей отбор, это дата вставшего сбора, а не
#: факт о канале (T-84), да и внутри одного дня в дампе сидит 8-13 тысяч
#: каналов — порядок по секундам последнего поста случаен. Обе колонки
#: по-прежнему выбираются и печатаются в карточке; убран порядок, не метрика.
SORTS = {
